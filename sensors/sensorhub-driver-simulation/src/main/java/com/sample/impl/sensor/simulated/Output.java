    ...
    public void run() {
        ...
        try {
            ...
            while (processSets) {
                ...
                // temperature; value will vary by less than 0.1 deg
                temp += variation(temp, tempRef, 0.001, 0.1);

                // pressure; value will vary by less than 20 hPa
                press += variation(press, pressRef, 0.001, 0.1);

                // wind speed; keep positive
                // vary between +/- 10 m/s
                windSpeed += variation(windSpeed, windSpeedRef, 0.001, 0.1);
                windSpeed = Math.max(windSpeed, 0.0);

                // wind direction; keep between 0 and 360 deg
                windDir += 2.0 * Math.random() - 1.0;
